...
mkdir -p "$OUT"
# Clean previous outputs so renamed/removed groups don't leave orphan files.
rm -f "$OUT"/*.txt "$OUT"/summary.json "$OUT"/index.html 2>/dev/null || true
...
# index.html is generated by mrt_cn_routes.py itself (grouped by tier, with a
# link back to the main site) so its naming stays in sync with the code.
